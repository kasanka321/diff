...
diff_match_patch.prototype.diff_main = function(text1, text2, opt_checklines,
  ...
  let ignore_diff = this.ignore(diffs);
 ...
 * @param {Array.<diff_match_patch.Diff>} diff Array of diff tuples.
 * @return {!Array.<!diff_match_patch.Diff>} Array of diff tuples.
 */
 diff_match_patch.prototype.ignore = function(diffs) {
  ...
  diffs.forEach(function(element){
    if(!element[1].match(reg)){
      ignore_diff.push(element);
    }else{
      // 修正前にあわせる（修正後にある単語で、無視して良い単語は描画しない）
      if(element[0] == 0 || element[0] == -1){
        ignore_diff.push([0,element[1]]);
      }
    }
  });
